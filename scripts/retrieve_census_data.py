...
if __name__ == "__main__":
    ...
    c = Census(api_key)
    county_census = c.acs5.state_county_blockgroup(
        fields=tuple(
            column_names.keys()),
    ...
    county_df['GEOID'] = county_df['state'] + county_df['county'] + \
        county_df['tract'] + county_df['block group']
    county_df.drop(
        columns=[
            'state',
            'county',
    ...
    county_merge['multi-family_with_2_-_4_units'] = county_merge[multi_family].sum(
        axis=1)
    county_merge['multi-family_with_5plus_units'] = county_merge[many_family].sum(
        axis=1)
